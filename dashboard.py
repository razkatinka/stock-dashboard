...
def _chart_layout(**overrides):
    """Return CHART_DEFAULTS merged with overrides (handles showlegend etc.)."""
...
def fetch_ticker_data(ticker: str) -> dict:
    ...
    fetches = [
        ...
        ("history",               lambda: t.history(period="1y")),
        ("history_ytd",           lambda: t.history(period="ytd")),
        ("institutional_holders", lambda: t.institutional_holders),
        ("recommendations",       lambda: t.recommendations),
        ("analyst_price_targets", lambda: t.analyst_price_targets),
    ]
    for key, fn in fetches:
        try:
            data[key] = fn()
        except Exception:
            data[key] = {} if key in ("info", "analyst_price_targets") else pd.DataFrame()
    return data
...
def calc_roic(financials: pd.DataFrame, balance_sheet: pd.DataFrame) -> float:
    try:
        ...
        if "Tax Rate For Calcs" in financials.index:
            tax = float(financials.loc["Tax Rate For Calcs"].iloc[0])
        elif "Income Tax Expense" in financials.index and "Pretax Income" in financials.index:
            pretax = float(financials.loc["Pretax Income"].iloc[0])
            tax_exp = float(financials.loc["Income Tax Expense"].iloc[0])
            if pretax != 0:
                tax = tax_exp / pretax

        ic_row = next((r for r in ["Invested Capital", "Total Assets"] if r in balance_sheet.index), None)
        if not ic_row:
            return float("nan")
...
def extract_catalysts_risks(summary: str, sector: str = "Default") -> tuple:
    ...
    while len(cats) < 3:
        cats.append(fb_cats[len(cats) % len(fb_cats)])
    while len(risks) < 3:
        risks.append(fb_risks[len(risks) % len(fb_risks)])
...
def chart_margins_vs_sector(info: dict, medians: dict) -> go.Figure:
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(
        y=labels, x=company, orientation="h", name="Company",
        marker_color=COLOR_GREEN, opacity=0.85,
        text=[f"{v:.1f}%" for v in company], textposition="outside",
    ))
    fig.add_trace(go.Bar(
        y=labels, x=sector, orientation="h", name="Sector Median",
...
def chart_ytd_performance(main_ticker: str, peers: list, ytd_histories: dict) -> go.Figure:
    ...
    labels_map = {"^IXIC": "NASDAQ", "^GSPC": "S&P 500"}
    results = []
...
def build_excel_export(ticker: str, data: dict, peer_data: dict) -> bytes:
    ...
    macd_data = calc_macd(close) if len(close) > 26 else {}
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        fund_rows = {
            "Metric": [
                "Last Price", "52W Return", "FY Revenue", "Forward EPS", "Market Cap",
        ...
        tech_rows = {
            "Metric": [
                "Beta", "RSI 14D", "MACD Signal", "50DMA", "200DMA",
                "52W High", "52W Low",
...
def render_fundamentals(ticker: str, data: dict) -> None:
    ...
    try:
        ...
        with c1:
            st.metric("מחיר אחרון", f"${price:.2f}" if price else "N/A")
        with c2:
            st.metric("תשואה 52W", fmt_pct(w52_chg))
        with c3:
            st.metric("הכנסות FY", fmt_currency(revenue, "B"))
        with c4:
            st.metric("Non-GAAP EPS", f"${fwd_eps:.2f}" if fwd_eps else "N/A")
        with c5:
            st.metric("שווי שוק", fmt_currency(mktcap, "B"))
        with c6:
            st.metric("Forward P/E", f"{float(fwd_pe):.1f}x" if fwd_pe else "N/A")
    ...
    try:
        ...
        with col_biz:
            for b in bullets:
                st.markdown(
                    f'<div dir="rtl" style="margin-bottom:8px; color:{COLOR_WHITE}; '
                    f'font-size:0.88rem;">• {b}</div>',
                    unsafe_allow_html=True,
                )
        with col_model:
            ...
            st.markdown(
                ...
                f'<div style="color:{COLOR_GREEN}; font-size:0.95rem; font-weight:600; '
                f'margin-top:8px; direction:rtl;">{model_text}</div>'
    ...
    with cc1:
        try:
            ...
            st.markdown(
                ...
                f'<span style="color:{COLOR_MUTED}">FCF: </span>{fmt_currency(safe_get(info,"freeCashflow"), "B")}</div>'
                f'<div dir="rtl" style="margin-bottom:6px;">'
                f'<span style="color:{COLOR_MUTED}">מזומן: </span>{fmt_currency(safe_get(info,"totalCash"), "B")}</div>'
                f'<div dir="rtl" style="margin-bottom:6px;">'
                f'<span style="color:{COLOR_MUTED}">Debt/Equity: </span>{de_str}</div>'
                f'<div dir="rtl">'
                f'<span style="color:{COLOR_MUTED}">Current Ratio: </span>{fmt_num(safe_get(info,"currentRatio"))}</div>'
                f'</div>',
                unsafe_allow_html=True,
            )
        except Exception:
    ...
    with cc2:
        try:
            ...
            spread_color = COLOR_GREEN if (not np.isnan(spread) and spread > 0) else COLOR_RED
            spread_str = f"{spread:+.1f}%" if not np.isnan(spread) else "N/A"
            st.markdown(
                f'<div class="card">'
                f'<div dir="rtl" style="margin-bottom:6px;">ROIC: '
                f'<span style="color:{COLOR_GREEN}; font-weight:600;">{fmt_num(roic)}%</span></div>'
                f'<div dir="rtl" style="margin-bottom:6px;">WACC (est.): '
                f'<span style="color:{COLOR_YELLOW}; font-weight:600;">{wacc_pct:.1f}%</span></div>'
                f'<div dir="rtl">Spread: '
                f'<span style="color:{spread_color}; font-weight:600;">{spread_str}</span></div>'
                f'</div>',
                unsafe_allow_html=True,
            )
        except Exception:
    ...
    with cc3:
        try:
            ...
            st.markdown(
                f'<div class="card">'
                f'<div dir="rtl" style="margin-bottom:6px;">'
                f'<span style="color:{COLOR_MUTED}">Buybacks: </span>{fmt_currency(buybacks, "B")}</div>'
                f'<div dir="rtl" style="margin-bottom:6px;">'
                f'<span style="color:{COLOR_MUTED}">Dividends: </span>{fmt_currency(divs_paid, "B")}</div>'
                f'<div dir="rtl">'
                f'<span style="color:{COLOR_MUTED}">R&D: </span>{fmt_currency(rnd_spend, "B")}</div>'
                f'</div>',
                unsafe_allow_html=True,
            )
        except Exception:
    ...
    # Store peer_data for export use
...
def render_technical(ticker: str, data: dict) -> None:
    ...
    try:
        c1, c2, c3, c4, c5, c6 = st.columns(6)
        with c1:
            st.metric("Beta", fmt_num(safe_get(info, "beta")))
        with c2:
            st.metric("RSI 14D", fmt_num(rsi_val))
        with c3:
            st.metric("MACD", macd_data.get("label", "N/A"))
        with c4:
    ...
    try:
        ...
        with col_table:
            if recs is not None and len(recs) > 0:
                ...
                if valid_cols:
                    rename_map = {
                        "period": "תקופה", "strongBuy": "Strong Buy", "buy": "Buy",
    ...
    try:
        ...
        if level_rows:
            ...
            styled = df_levels.style.map(_style_status, subset=["סטטוס"])
        ...
        macd_color = COLOR_GREEN if macd_lbl == "BULLISH" else COLOR_RED
        st.markdown(
            ...
            f' &nbsp;|&nbsp; {rsi_str}'
            f' &nbsp;|&nbsp; MACD: <span style="color:{macd_color}">{macd_lbl}</span>'
            f'</div>',
            unsafe_allow_html=True,
        )
    except Exception as e:
    ...
    try:
        if inst is not None and len(inst) > 0:
            ...
            col_rename = {}
            if "Holder" in top5.columns:
                col_rename["Holder"] = "מחזיק"
            if "Value" in top5.columns:
                col_rename["Value"] = "שווי ($)"
            if "% Out" in top5.columns:
                col_rename["% Out"] = "% החזקה"
    ...
    try:
        ...
        bear_t = price * ((1 + bear_growth) ** 3) * 0.80

        cc1, cc2, cc3 = st.columns(3)
        with cc1:
            ret = (bull_t / price - 1) * 100
            st.markdown(
                f'<div class="scenario-bull">'
                f'<div style="color:{COLOR_GREEN}; font-size:1.05rem; font-weight:700;">BULL 🐂</div>'
                f'<div style="color:{COLOR_MUTED}; font-size:0.78rem; margin-top:4px;">הסתברות: 25%</div>'
                f'<div style="font-size:1.6rem; font-weight:700; color:{COLOR_GREEN}; margin:10px 0;">${bull_t:.2f}</div>'
                f'<div style="color:{COLOR_GREEN}; font-size:0.85rem;">{ret:+.1f}% vs. נוכחי</div>'
                f'<div style="color:{COLOR_MUTED}; font-size:0.72rem; margin-top:6px;">'
                f'{bull_growth*100:.0f}%/y | PE ×1.10</div></div>',
                unsafe_allow_html=True,
            )
        with cc2:
            ret = (base_t / price - 1) * 100
            st.markdown(
                f'<div class="scenario-base">'
                f'<div style="color:{COLOR_YELLOW}; font-size:1.05rem; font-weight:700;">BASE 📊</div>'
                f'<div style="color:{COLOR_MUTED}; font-size:0.78rem; margin-top:4px;">הסתברות: 50%</div>'
                f'<div style="font-size:1.6rem; font-weight:700; color:{COLOR_YELLOW}; margin:10px 0;">${base_t:.2f}</div>'
                f'<div style="color:{COLOR_YELLOW}; font-size:0.85rem;">{ret:+.1f}% vs. נוכחי</div>'
                f'<div style="color:{COLOR_MUTED}; font-size:0.72rem; margin-top:6px;">'
                f'{base_growth*100:.0f}%/y | PE ×1.00</div></div>',
                unsafe_allow_html=True,
            )
        with cc3:
            ret = (bear_t / price - 1) * 100
            st.markdown(
                f'<div class="scenario-bear">'
                f'<div style="color:{COLOR_RED}; font-size:1.05rem; font-weight:700;">BEAR 🐻</div>'
                f'<div style="color:{COLOR_MUTED}; font-size:0.78rem; margin-top:4px;">הסתברות: 25%</div>'
                f'<div style="font-size:1.6rem; font-weight:700; color:{COLOR_RED}; margin:10px 0;">${bear_t:.2f}</div>'
                f'<div style="color:{COLOR_RED}; font-size:0.85rem;">{ret:+.1f}% vs. נוכחי</div>'
                f'<div style="color:{COLOR_MUTED}; font-size:0.72rem; margin-top:6px;">'
                f'{bear_growth*100:.0f}%/y | PE ×0.80</div></div>',
                unsafe_allow_html=True,
            )
    except Exception as e:
...
def main() -> None:
    ...
    # Build export data (uses cached peer fetch so it's fast)
